...
| 2026-09-04 | Codex | 根据用户实机截图继续修正资产可见性：不再要求进入详情才能发现多图，按季资产汇总与资产库分类表均直接展示最多4张当前版本缩略图及余量；构建通过后先关闭旧软件，再重编并启动新版 exe。 |
| 2026-09-04 | Codex | 用户确认 R003-04 标准左侧面身份特写，通过 MCP 累计升级并确认 R003 v004；当前包含正脸、左右三分之四、左侧面4张，下一项为右侧面。 |
| 2026-09-04 | Codex | 用户确认 R003-05 标准右侧面身份特写，通过 MCP 累计升级并确认 R003 v005；当前5张基础面部角度已齐，下一项为轻俯角。 |
