...
- **版本**：v0.5.7（`VERSION` 文件实测 `v0.5.7`）。
- **最后 git 提交**：`9cdd751 release: v0.5.7`（实测 `git log -1`）。
- ⚠️ **所有近期工作均为未提交状态**：实测 `git status --porcelain` 变更中 **426 条删除为已固定的历史事实**（WebView2 缓存），另有 M（修改）与 ??（未跟踪）若干条 —— 后两者为**动态值**，会随 `.tmp-mcp-*` 临时日志持续增长（仅代表测量时刻），**无 git 提交保护，存在丢失风险 —— 这是当前最高优先级风险。** 详见下方「git 工作区状态与提交纪律」子节。
...
- 暂停分镜生图；先按 65 项完整资产清单补齐待产出/需修改资产。R003-01 陆江仙中性正脸身份控制特写已由用户确认并通过 MCP 入库为 v001；下一张生产左三分之四头部特写，继续逐张确认所需角度、结构控制和半身衔接母版；随后按 P0 身份/风格/空间/核心道具顺序推进。
