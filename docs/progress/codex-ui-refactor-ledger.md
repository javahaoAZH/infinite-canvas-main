...
| [x] | R003-05 右侧面身份控制特写入库 | 2026-09-04；用户目视确认后经 MCP 累计绑定并确认 R003 v005，当前版本保留正脸、左右三分之四、左右侧面共5张，顶层交付件同步，剩余5项未完成。 |
...
| 2026-09-04 | Qoder | 实机闭环验证通过：运行中 exe 页面加载的 CSS 已含 close 规则（计算样式白字+深胶囊）——上一轮「不可见」实为 Codex 通道在我修复后又重编启动了旧前端产物导致回滚。本轮同时加切换按钮贴图定位（clamp 跟随图片边缘、窄窗贴边），并在 18080 实测 close/switch 计算样式与位置后提交。注意：Qoder 与 Codex 通道并行重编 exe 会互相覆盖，重编前先确认对方没在跑。 |
| 2026-09-04 | Qoder | 标题栏当前项目标签 ⋯ 菜单对接真实项目功能：漫剧项目=打开项目/导出分镜稿 Markdown/跳资产清单；画布项目=打开画布；通用项保留新对话与设置；上下文改订阅漫剧/画布 store（原来只读快照不响应）。 |
| 2026-09-04 | Qoder | 预览浮层第三轮：DOM 实测 close（ant-image-preview-close）与前后切换（ant-image-preview-switch-prev/next）虽渲染但颜色为深色不可见；统一加深色胶囊底+反白+hover；同时 Win11 原生层钉死 DWM 标题/边框颜色（属性 35/34=深色），修掉暗色模式顶边白色色带；均经浏览器计算样式实测（close/switch 前景白、背景深）+ exe 重编重启。 |
