...
- 百炼渠道后续待办：① `cosyvoice-v2` 等语音合成模型暂未接入（当前仅支持 qwen3-tts 系列）；② 百炼无浏览器直连能力，未登录一律拒绝，后续可评估与匿名代理打通；③ 图生视频首帧目前以 Base64 data URL 内联传输，请求体较大，后续可优化为先上传 OSS 再以 URL 传参；④ 百炼视频完成后的临时 OSS 链接需持久化转存结果回写服务端任务（当前仅当次会话内转存，刷新/过期后重新打开会重复下载或失败）。
