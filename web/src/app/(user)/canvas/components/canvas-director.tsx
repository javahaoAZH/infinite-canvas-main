...
export function CanvasDirector({
    ...
    // 兜底：iframe 加载失败时 postMessage 关闭不可用，Esc 同样可以退出。
    useEffect(() => {
    ...
    return (
        <div className="fixed inset-0 z-[2000]">
            ...
            <button
                ...
                title="关闭导演台（Esc）"
                onClick={onClose}
                className="absolute right-4 top-4 flex size-9 cursor-pointer items-center justify-center rounded-lg border transition-colors"
                style={{ background: colors.toolbar.panel, borderColor: colors.toolbar.border, color: colors.toolbar.item }}
                onMouseEnter={(event) => (event.currentTarget.style.background = colors.toolbar.itemHover)}
                onMouseLeave={(event) => (event.currentTarget.style.background = colors.toolbar.panel)}
            >
                <X className="size-4" />
            </button>
        </div>
