...
// 上传请求永不 settle 时，带 0 时长的 loading toast 会永久卡死；超过阈值直接报错，让调用方的 catch/finally 关闭 toast。
function withUploadTimeout<T>(promise: Promise<T>, label: string, timeoutMs: number): Promise<T> {
...
function InfiniteCanvasPage({ projectId }: { projectId: string }) {
    ...
    const getCanvasCenter = useCallback(() => {
        const rect = containerRef.current?.getBoundingClientRect();
        // 按已有节点数做级联偏移，避免连续新建的节点完全重叠在视口中心。
        const offset = 24 + (nodesRef.current.length % 6) * 24;
        return screenToCanvas((rect?.left || 0) + (rect?.width || size.width) / 2 + offset, (rect?.top || 0) + (rect?.height || size.height) / 2 + offset);
    ...
    const createImageFileNode = useCallback(async (file: File, position: Position, choosePanoramaImport = false) => {
        ...
        try {
            const image = await withUploadTimeout(uploadImage(file), "图片上传", IMAGE_UPLOAD_TIMEOUT_MS);
    ...
    const createVideoFileNode = useCallback(async (file: File, position: Position) => {
        ...
        try {
            const video = await withUploadTimeout(uploadMediaFile(file, "video"), "视频上传", MEDIA_UPLOAD_TIMEOUT_MS);
    ...
    const createAudioFileNode = useCallback(async (file: File, position: Position) => {
        ...
        try {
            const audio = await withUploadTimeout(uploadMediaFile(file, "audio"), "音频上传", MEDIA_UPLOAD_TIMEOUT_MS);
