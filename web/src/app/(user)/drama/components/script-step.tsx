...
import { SCRIPT_STRUCTURE_SYSTEM_PROMPT } from "@/app/(user)/drama/prompts";
...
type StructuredShot = { description?: string; dialogue?: string; seconds?: number };
        ...
        try {
            const response = await fetch(aiApiUrl(textConfig, "/chat/completions"), {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        { role: "system", content: SCRIPT_STRUCTURE_SYSTEM_PROMPT },
            ...
            const shots = structured.shots!.map((shot) => newDramaShot({ description: shot.description!, dialogue: shot.dialogue || "", seconds: shot.seconds }));
